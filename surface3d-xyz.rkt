...
;; x -> f(y,z)
(define ((surface3d-render-proc/x f samples color style line-color line-width line-style alpha label)
         area)
  (match-define (vector x-ivl y-ivl z-ivl) (send area get-bounds-rect))
  (define num (animated-samples samples))
  (define sample (f (vector y-ivl z-ivl) (vector num num)))
  ...
  (let* ([flonum-ok?  (flonum-ok-for-3d? x-min x-max y-min y-max z-min z-max)]
    ...
    (for-2d-sample
     (ya yb za zb x1 x2 x3 x4) sample
     (send area put-polygon
           (list (vector x1 ya za) (vector x2 yb za) (vector x3 yb zb) (vector x4 ya zb)))))
...
(defproc (surface3d/x
  ...
  (define g (2d-function->sampler f (vector y-ivl z-ivl)))
  (renderer3d (vector x-ivl y-ivl z-ivl)
              (surface3d-bounds-fun g samples)
              default-ticks-fun
              (surface3d-render-proc/x g samples color style
                                     line-color line-width line-style alpha label)))

;; y -> f(x,z)
(define ((surface3d-render-proc/y f samples color style line-color line-width line-style alpha label)
         area)
  (match-define (vector x-ivl y-ivl z-ivl) (send area get-bounds-rect))
  (define num (animated-samples samples))
  (define sample (f (vector x-ivl z-ivl) (vector num num)))
  ...
  (let* ([flonum-ok?  (flonum-ok-for-3d? x-min x-max y-min y-max z-min z-max)]
         [sample      (if flonum-ok? (2d-sample-exact->inexact sample) sample)])
    (for-2d-sample
     (xa xb za zb y1 y2 y3 y4) sample
     (send area put-polygon
           (list (vector xa y1 za) (vector xb y2 za) (vector xb y3 zb) (vector xa y4 zb)))))
...
(defproc (surface3d/y
  ...
  (define y-ivl (ivl y-min y-max))
  (define z-ivl (ivl z-min z-max))
  (define g (2d-function->sampler f (vector x-ivl z-ivl)))
  (renderer3d (vector x-ivl y-ivl z-ivl)
              (surface3d-bounds-fun g samples)
              default-ticks-fun
              (surface3d-render-proc/y g samples color style
                                     line-color line-width line-style alpha label)))


;; z -> f(x,y)
(define ((surface3d-render-proc/z f samples color style line-color line-width line-style alpha label)
         area)
  (match-define (vector x-ivl y-ivl z-ivl) (send area get-bounds-rect))
  (define num (animated-samples samples))
...
(defproc (surface3d/z
  ...
  (renderer3d (vector x-ivl y-ivl z-ivl)
              (surface3d-bounds-fun g samples)
              default-ticks-fun
              (surface3d-render-proc/z g samples color style
                                     line-color line-width line-style alpha label)))
